export async function POST(req) {
  try {
    ...
    await db.execute(
      `UPDATE user_notification_channels SET verification_status='verified', is_active=1 WHERE address=? AND channel_id=(SELECT id FROM channels WHERE name='whatsapp')`,
      [number]
    );
