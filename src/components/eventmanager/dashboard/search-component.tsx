      ...
      <div className={`relative ${className}`} ref={searchRef}>
        ...
        {showSuggestions && recentSearches.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-xl z-50 overflow-hidden">
            <div className="py-2">
              {recentSearches
                .filter((search) => searchQuery === "" || search.toLowerCase().includes(searchQuery.toLowerCase()))
              ...
              {/* No results message */}
              {searchQuery &&
                recentSearches.filter((search) => search.toLowerCase().includes(searchQuery.toLowerCase())).length ===
                  0 && <div className="px-4 py-3 text-sm text-gray-500 text-center">No recent searches found</div>}
            </div>
          </div>
        )}
