...
export function SortingDropdown({ onSelectionChange, page = "default" }: SortingDropdownProps) {
  const [globalSelected, setGlobalSelected] = useState(false)
  const [selectedZones, setSelectedZones] = useState<ZoneOption[]>([])
  
  const showGlobalOption = page !== "pending-events"
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        <TooltipTrigger asChild>
          <div>
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-56 bg-popover border-border shadow-lg" align="start">
                <DropdownMenuGroup>
                  {/* Show Global option only if not on pending-events page */}
                  {showGlobalOption && (
                    <DropdownMenuItem
                      className="flex items-center gap-2 cursor-pointer"
                      onSelect={(e) => e.preventDefault()}
                  ...
                  {/* Zones submenu - always show, but as main content on pending-events */}
                  {showGlobalOption ? (
                    <DropdownMenuSub>
                      <DropdownMenuSubTrigger>Zones</DropdownMenuSubTrigger>
                      <DropdownMenuSubContent className="w-48 bg-popover border-border shadow-lg">
                        <ZoneMenuItems selectedZones={selectedZones} onZoneToggle={handleZoneToggle} />
                      </DropdownMenuSubContent>
                    </DropdownMenuSub>
                  ) : (
                    // On pending-events page, show zones directly without submenu
                    <ZoneMenuItems selectedZones={selectedZones} onZoneToggle={handleZoneToggle} />
                  )}
                </DropdownMenuGroup>
              </DropdownMenuContent>
        ...
        <TooltipContent>
          <p>{showGlobalOption ? "Select global or zone events" : "Select zone"}</p>
        </TooltipContent>
      </Tooltip>
    </TooltipProvider>
  )
...
// Helper component for zone menu items
function ZoneMenuItems({ 
  selectedZones, 
  onZoneToggle 
}: { 
  selectedZones: ZoneOption[]
  onZoneToggle: (zone: ZoneOption, checked: boolean) => void 
}) {
  const zones: ZoneOption[] = ["zone-1", "zone-2", "zone-3", "zone-4", "zone-5"]
    ...
    <>
      {zones.map((zone) => (
        <DropdownMenuItem
          ...
          className="flex items-center gap-2 cursor-pointer"
          onSelect={(e) => e.preventDefault()}
          onClick={() => onZoneToggle(zone, !selectedZones.includes(zone))}
        >
          <Checkbox
            checked={selectedZones.includes(zone)}
            onCheckedChange={(checked) => onZoneToggle(zone, checked as boolean)}
          />
          <span>Zone {zone.split("-")[1]}</span>
        </DropdownMenuItem>
      ))}
    </>
  )
