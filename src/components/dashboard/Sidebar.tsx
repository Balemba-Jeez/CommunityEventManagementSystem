...
const liveStreamItems = [
  { label: 'Now Live', href: '/events/live/now', badge: 'LIVE' },
  { label: 'Recent Live', href: '/events/live/recent' },
  { label: 'Upcoming Live', href: '/events/live/upcoming' },
];
...
export const Sidebar = ({ collapsed, onToggle }: SidebarProps) => {
  const [liveStreamsExpanded, setLiveStreamsExpanded] = useState(false);

  const toggleLiveStreams = () => {
    setLiveStreamsExpanded(prev => !prev);
  };
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              to="/events/live/now"
              className="flex items-center justify-center w-full h-12 px-3 transition-colors hover:bg-gray-100 hover:text-black text-gray-700 rounded-lg"
            >
              <Radio className="h-5 w-5 flex-shrink-0 text-gray-600" />
            </Link>
          </TooltipTrigger>
          <TooltipContent side="right" className="ml-2">
            <p>Live Streams</p>
  ...
  return (
    <div>
      <button
        ...
        className="flex items-center w-full h-12 px-3 transition-colors hover:bg-gray-100 hover:text-black text-gray-700 rounded-lg"
      >
        <Radio className="h-5 w-5 mr-3 flex-shrink-0 text-gray-600" />
        <span className="flex-1 text-left font-medium">Live Streams</span>
        {liveStreamsExpanded ? (
      ...
      {liveStreamsExpanded && (
        <div className="ml-8 space-y-1 mt-1">
          {liveStreamItems.map((item) => (
            <Link
              key={item.href}
              to={item.href}
              className="flex items-center w-full h-10 px-3 transition-colors hover:bg-gray-100 hover:text-black text-gray-600 rounded-lg text-sm"
            >
