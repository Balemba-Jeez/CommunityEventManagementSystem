...
export const Sidebar = ({ collapsed, onToggle }: SidebarProps) => {
  ...
  // Check if any live stream route is active
  const isLiveStreamActive = () => {
    return liveStreamItems.some(item => location.pathname === item.href);
  };
...
const SidebarItem = ({ 
  ...
  const active = isActiveRoute(href); // Dynamic active state
...
const LiveStreamsDropdown = () => {
  const isActive = isLiveStreamActive();
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <Link
              to="/events/live/now"
              className={`flex items-center justify-center w-full h-12 px-3 transition-colors ${
                isActive
                  ? 'font-semibold text-[#2C3E94] border-l-2 border-[#2C3E94]'
                  : 'hover:bg-gray-100 hover:text-black text-gray-700 rounded-lg'
              }`}
            >
              <Radio className={`h-5 w-5 flex-shrink-0 ${
                isActive ? 'text-[#2C3E94]' : 'text-gray-600'
              }`} />
            </Link>
          </TooltipTrigger>
          <TooltipContent side="right" className="ml-2">
  ...
  return (
    <div>
      ...
      >
        <Radio className={`h-5 w-5 mr-3 flex-shrink-0 ${
          isActive ? 'text-[#2C3E94]' : 'text-gray-600'
        }`} />
        ...
        <div className="ml-8 space-y-1 mt-1">
          {liveStreamItems.map((item) => (
            <Link
              key={item.href}
              to={item.href}
              className={`flex items-center w-full h-10 px-3 transition-colors ${
                isActiveRoute(item.href)
                  ? 'font-semibold text-[#2C3E94] bg-blue-50'
